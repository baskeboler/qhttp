...
namespace {
///////////////////////////////////////////////////////////////////////////////
bool appendTrusted(QSslConfiguration &sslconfig, const CertificateList &cas) {
  if (cas.isEmpty())
    return false;
...
QSslCertificate loadCertificate(const QString &filePath,
                                QSsl::EncodingFormat fmt) {
  QFile f(filePath);
...
QSslKey loadKey(const QString &filePath, QSsl::KeyAlgorithm alg,
                QSsl::EncodingFormat fmt) {
  QFile f(filePath);
  if (f.open(QFile::ReadOnly)) {
...
ssl::Socket::Socket(QObject *parent) : QSslSocket(parent) {
  ...
  QObject::connect(this, serrFunc,
                   [this](const auto &errors) { this->onSslErrors(errors); });

  QObject::connect(
      this, &QSslSocket::peerVerifyError,
      [this](const QSslError &err) { this->onPeerVerifyError(err); });
}

void Socket::onSslErrors(const QList<QSslError> &errors) {
  qDebug("ssl errors from %s:%d",
         qPrintable(Stringify::toStringV4(peerAddress())), peerPort());

  for (const auto &err : errors) {
    qDebug("  (%d): %s", static_cast<int>(err.error()),
           qPrintable(err.errorString()));
  }
}

void Socket::onPeerVerifyError(const QSslError &err) {
  qDebug("peer certificate failed (%d): %s", static_cast<int>(err.error()),
         qPrintable(err.errorString()));
}

void ssl::Socket::setup(const CertificateList &cas) {
...
void ssl::Socket::acceptSelfSignedCertificates(const CertificateList &certs) {
  if (certs.isEmpty())
    return;
