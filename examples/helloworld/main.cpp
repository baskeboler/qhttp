...
void runWeatherClient(const QString &cityName) {
  ...
  client.get(weatherUrl, [&httpBody](QHttpResponse *res) {
    ...
    res->onData(
        [&httpBody](const QByteArray &chunk) { httpBody.append(chunk); });
...
int main(int argc, char **argv) {
  ...
  parser.setApplicationDescription(
      "a HelloWorld example for http client and server.");
  parser.addHelpOption();
  parser.addVersionOption();
  parser.addPositionalArgument(
      "mode", "working mode: server, client or weather. default: server");
