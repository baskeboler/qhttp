...
signals:
   ...
   * @note This may be emitted zero or more times depending on the transfer
   * type.
   * @see onData();
   */
...
public:
   ...
   * @param dataHandler a std::function or lambda handler to receive incoming
   * data.
   ...
   * @param endHandler a std::function or lambda handler to receive end
   * notification.
...
public:
  /** tries to collect all the incoming data internally.  @note if you call
   * this method, data() signal won't be emitted and onData() will have no
   * effect.
   *
   * @param atMost maximum acceptable incoming data. if the incoming data
   * exceeds this value, the connection won't read any more data and end()
   * signal will be emitted.  default value (-1) means read data as
   * "content-length" or unlimited if the body size is unknown.
   */
...
public:
  ...
  explicit QHttpAbstractInput(QObject *parent = nullptr);
...
public:
  ...
  template <typename T> void addHeaderValue(const QByteArray &field, T value);

  /** adds an HTTP header to the packet.
   * @note this method does not actually write anything to socket, just
...
signals:
  ...
  /** Emitted when the packet is finished and reports if it was the last
   * packet.
...
protected:
  explicit QHttpAbstractOutput(QObject *parent = nullptr);
