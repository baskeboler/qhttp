 ...
 * This class internally handles the memory management and life cycle of
 * QHttpRequest and QHttpResponse instances. you do not have to manually delete
...
public:
  explicit QHttpClient(QObject *parent = nullptr,
                       TBackend backendType = ETcpSocket);
   ...
   * @param url specifies server's address, port and optional path and query
   * strings. if url starts with socket:// the request will be made on
   * QLocalSocket, otherwise normal QTcpSocket will be used.
   ...
   */
  bool request(THttpMethod method, QUrl url, const TRequestHandler &reqHandler,
               const TResponseHandler &resHandler);

  /** tries to connect to a HTTP server.
   *  when the connection is made, a default request handler is called
   *  automatically (simply calls req->end()) and when the response is ready,
   *  resHandler will be called.
   * @note httpConnected() and newResponse() won't be emitted.
   *
   * @param method an HTTP method, ex: GET, POST, ...
   * @param url specifies server's address, port and optional path and query
  ...
  /// helper
  template <class... Args> inline bool get(Args &&... args) {
    return request(THttpMethod::EHTTP_GET, std::forward<Args &&>(args)...);
  }

  template <class... Args> inline bool post(Args &&... args) {
    return request(THttpMethod::EHTTP_POST, std::forward<Args &&>(args)...);
  }

  template <class... Args> inline bool put(Args &&... args) {
    return request(THttpMethod::EHTTP_PUT, std::forward<Args &&>(args)...);
  }
   ...
   * each (already opened) connection will be forcefully closed after this
   * timeout.
   ...
   *  the @sa timedOut(quint32) signal will be emitted and connection will be
   *  killed.
...
signals:
  ...
  /// emitted when fails to connect to a HTTP server.
  /// @sa setConnectingTimeOut()
  void connectingTimeOut();

protected:
  /** called when a new HTTP connection is established.
   * you can overload this method, the default implementaion only emits
   * connected().
